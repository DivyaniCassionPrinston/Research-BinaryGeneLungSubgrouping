...
# Load the dataset that contains only the most informative features selected earlier.
...
# Determine the optimal number of PCA components using cross-validation with Gaussian Naive Bayes.
print(f"\n{'=' * 60}")
...
for n_comp in component_range:
    ...
    # Evaluate Gaussian Naive Bayes performance using cross-validation
    nb_temp = GaussianNB()
    scores = cross_val_score(nb_temp, X_pca_temp, y, cv=10, scoring="accuracy", n_jobs=-1)
...
# Perform stratified 10-fold cross-validation with PCA + Gaussian Naive Bayes.
n_splits = 10
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), start=1):
    ...
    # Train Gaussian Naive Bayes on PCA-reduced features
    start_time = time.time()
    memory_before = process.memory_info().rss
    ...
    nb_memory.append((memory_after - memory_before) / (1024**2))
    ...
    print(f"  NB Training: {training_time:.2f}s | Accuracy: {accuracy:.4f}")
    print(f"  Memory - PCA: {pca_memory[-1]:.2f}MB | NB: {nb_memory[-1]:.2f}MB\n")
...
print(f"Average NB Training Time: {np.mean(training_times):.2f}s ± {np.std(training_times):.2f}s")
...
print(f"Average NB Memory: {np.mean(nb_memory):.2f}MB ± {np.std(nb_memory):.2f}MB")

# ==================== Overall Model Evaluation ====================
# Evaluate model using all folds combined
print(f"\n{'=' * 60}")
...
# Apply PCA on full dataset and train Gaussian Naive Bayes for deployment
print(f"\n{'=' * 60}")
...
start_time = time.time()
final_nb = GaussianNB()
final_nb.fit(X_pca_full, y)
final_training_time = time.time() - start_time

print(f"Naive Bayes training completed in {final_training_time:.2f} seconds.")
final_accuracy = final_nb.score(X_pca_full, y)
